...
-e,--edit=X	  use X as the file to be edited; by default, the file
		  ...
		  editing, or, if there are no errors, the main source
		  file.
...
mk uses [texi2dvi](ctan.org/tex-archive/macros/texinfo/latest)
for compilation, [vpp](www.ctan.org/pkg/vpp) for viewing and printing. Any
...
altdir=	
	...
	your LaTeX file in |/Docs/myfile.tex| you can set |altdir| to
	|/Docs| and run mk from any directory with:
...
handle_options() {
   ...
   options=$(getopt \
      ...
      -l noverbose,noview,preview,print,printer:,ps,rc::,verbose,version,view -- "$@"
   ) || exit 1
   eval set -- "$options"
   while true; do
      if $first; then
	 ...
	 if [[ -n $rcfile ]]; then
      ...
      (	  --noverbose)	# be quiet (this is the default)
			...
			shift
			;;
      (-r|--rc)		# set rc file to X, but this must be the first option!
			# If X is absent, don’t read any rc file.
      ...
      # mk specific options:
      (-f|--formatter)	# Use X as the formatter: tex, latex, pdflatex,
			# et cetera
			formatter="$2"
			excheck "$formatter"
			shift 2
			;;
      (-e|--edit)	# use X as the file to be edited; by default, the file
			# containing the compilation error is presented for
			# editing, or, if there are no errors, the main source
			# file.
			edit="${2/\~/$HOME}"
      ...
      (-b|--batch)	# run in batch using X as a printing command for vpp.
			...
			shift 2
			;;
      (-p|--printer)	# print to printer named X
			printer="$2"
			shift 2
			;;
      (	  --view)	# view the document after compilation
